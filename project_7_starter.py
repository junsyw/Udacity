...
# In the graph we see high autocorrelation values for the different labels (up to ~0.75 for target_1), so there is a corralation between these shifted labels
...
# The feature in the first node does not need to be the one with the highest importance. In this graph we can see that the feature "dispersion_20d" has the higher information gain as it is used in multiple nodes as criteria for the splitting
# 
...
# The model seem to overfit as the validation accuracy seems to decline
# 
...
def bagging_classifier(n_estimators, max_samples, max_features, parameters):
    ...
    # TODO: Implement
    bagging_clf = BaggingClassifier(base_estimator=DecisionTreeClassifier(criterion=parameters['criterion'],min_samples_leaf= parameters['min_samples_leaf']),                                    n_estimators=n_estimators,                                     max_samples=max_samples,                                     max_features=max_features,                                    oob_score = parameters['oob_score'],                                    n_jobs = parameters['n_jobs'],                                    random_state = parameters['random_state']
                                   )
...
def calculate_oob_score(classifiers):
    ...
    return np.mean([o.oob_score_ for o in classifiers])
...
def non_overlapping_estimators(x, y, classifiers, n_skip_samples):
    ...
    for i in range(len(classifiers)):
            non_overlapping_x ,non_overlapping_y = non_overlapping_samples(x, y, n_skip_samples, start_i=i)
            fit_classifiers.append( classifiers[i].fit(non_overlapping_x,non_overlapping_y))
